...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        auto,
        block/.style={draw, rectangle, minimum height=2.5em, minimum width=3em, align=center, thick},
        filter/.style={draw, rectangle, minimum height=2em, minimum width=2.5em, align=center},
    ...
    \node [sum, right=0.8cm of input] (sum1) {$\Sigma$};
    \node [block, right=1cm of sum1] (pi) {PI\\$G_{c,\theta}$};
    \node [sum, right=1cm of pi] (sum2) {$\Sigma$};
    \node [block, right=1cm of sum2] (pid) {Rate\\PID};
    \node [block, right=1cm of pid] (plant) {Plant\\$G_p(s)$};
    \node [output, right=1.5cm of plant] (output) {};

    % Forward connections
    \draw [->, thick] (input) -- node[above] {$\theta_{ref}$} (sum1);
    \draw [->, thick] (sum1) -- node[above] {$e_\theta$} (pi);
    \draw [->, thick] (pi) -- node[above] {$\omega_{sp}$} (sum2);
    \draw [->, thick] (sum2) -- node[above] {$e_\omega$} (pid);
    \draw [->, thick] (pid) -- node[above] {$u$} (plant);
    \draw [->, thick] (plant) -- node[above, name=y] {$\omega$} (output);

    % Rate Loop Feedback (Gyroscope)
    \node [block, below=1cm of plant] (gyro) {Gyro\\$H_\omega$};
    \draw [->, thick] (plant.east) -- ++(0.6,0) |- (gyro.east);
    \draw [->, thick] (gyro.west) -| (sum2.south) node[pos=0.9, left] {$-$};

    % Complementary Filter Block
    \node [filter, below=1.8cm of pi] (hpf) {HPF\\$(1-\alpha)$};
    \node [filter, right=0.6cm of hpf] (int) {$\int$};
    \node [sum, right=0.6cm of int] (sumcf) {$+$};
    
    \node [filter, below=0.9cm of sumcf] (lpf) {LPF\\$\alpha$};
    \node [block, below=0.9cm of lpf] (accel) {Accel\\$H_a$};
    
    % CF Connections
    \draw [->, thick] (gyro.west) -- ++(-0.3,0) |- (hpf.north);
    \draw [->, thick] (hpf) -- (int);
    \draw [->, thick] (int) -- (sumcf);
    
    \draw [->, thick] (accel) -- (lpf);
    \draw [->, thick] (lpf) -- (sumcf);
    
    % Angle estimate output and feedback
    \draw [->, thick] (sumcf.east) -- ++(0.8,0) node[right] {$\hat{\theta}$};
    \draw [->, thick] (sumcf.east) -- ++(0.5,0) |- ++(-7.5,3.5) -| (sum1.south) node[pos=0.98, left] {$-$};

    % Dashed boxes with labels
    \draw [gray, thick, dashed] ($(sum2.north west)+(-0.3,0.4)$) rectangle ($(gyro.south east)+(0.3,-0.3)$);
    \node[gray] at ($(sum2.north)+(1.5,0.7)$) {\footnotesize Inner Rate Loop};

    \draw [gray, thick, dashed] ($(hpf.north west)+(-0.3,0.4)$) rectangle ($(accel.south east)+(0.9,-0.3)$);
    \node[gray] at ($(accel.south)+(0.5,-0.6)$) {\footnotesize Complementary Filter};
