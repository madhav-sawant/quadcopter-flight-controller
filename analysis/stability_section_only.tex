...
The quadcopter rotational dynamics are modeled based on Newton's Euler equations, simplified for single-axis rotation. The plant transfer function $G_p(s)$ relates the controller output to the angular velocity.
...
Figure \ref{fig:rate_block} shows the rate mode control loop structure.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{rate_mode_block_diagram.png}
\caption{Rate Mode Control Loop Block Diagram. Signals: $\omega_{ref}$ = rate setpoint (deg/s), $e$ = error, $u$ = control output, $\omega$ = measured angular rate, $G_c(s)$ = PID controller, $G_p(s)$ = plant.}
...
Stability is determined by the poles of the closed-loop system, which are the roots of the characteristic equation $1 + G_c(s)G_p(s) = 0$.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{rate_mode_splane.png}
\caption{S-plane pole-zero map showing all poles in the stable Left Half Plane.}
\label{fig:splane}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{rate_mode_damping.png}
\caption{Damping ratio analysis showing $\zeta = 0.71$, indicating near-optimal damping.}
\label{fig:damping}
\end{figure}
...
Figure \ref{fig:angle_block} illustrates the complete angle mode control architecture.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{angle_mode_block_diagram.png}
\caption{Angle Mode Cascade Control with Complementary Filter. Signals: $\theta_{ref}$ = angle setpoint (deg), $e_\theta$ = angle error, $\omega_{sp}$ = rate setpoint, $e_\omega$ = rate error, $u$ = control output, $\omega$ = angular rate, $\hat{\theta}$ = estimated angle from complementary filter.}
\label{fig:angle_block}
\end{figure}

\subsubsection{Complementary Filter}

The attitude feedback is obtained through a complementary filter, which combines gyroscope and accelerometer data:
\begin{equation}
...
In the cascade architecture, the outer angle loop uses the closed-loop rate system $T(s)$ (derived in the previous section) as its inner plant. The angle plant $G_{p,\theta}(s)$ is the rate closed-loop followed by integration from rate to angle:
...
\textbf{Pole} & \textbf{Location} & \textbf{$\zeta$} & \textbf{$\omega_n$ (rad/s)} \\
\hline
1, 2 & $-100.5 \pm 98.3j$ & 0.72 & 140.6 \\
3, 4 & $-6.7 \pm 2.0j$ & 0.96 & 7.0 \\
5 & $-0.52$ (real) & -- & -- \\
6 & $-0.10$ (real) & -- & -- \\
\hline
\end{tabular}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{angle_mode_splane.png}
\caption{S-plane showing all poles in stable region (PI Controller).}
\label{fig:angle_splane}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{angle_mode_damping.png}
\caption{Damping ratio analysis showing dominant poles with $\zeta = 0.96$.}
\label{fig:angle_damping}
\end{figure}
...
% ===========================================================================
% END OF STABILITY ANALYSIS SECTION
% ===========================================================================
